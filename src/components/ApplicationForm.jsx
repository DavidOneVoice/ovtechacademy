...
export default function ApplicationForm({ type = "scholarship" }) {
  ...
  return <main className="academy-registration"><Navbar />
    ...
    {done ? <section className="academy-completion" role="status"><span className="academy-eyebrow">Application received</span><h2>You’ve taken the first step.</h2><p>Your {course.title} scholarship application is saved for our admissions team to review. We’ll contact you by email or WhatsApp.</p><p>Application reference: <strong>{applicationId}</strong></p><a className="academy-button" href="/courses">Explore the courses</a></section> :
    <div className="academy-registration-layout">
      <form className="academy-registration-form" onSubmit={submit}>
        ...
        {!scholarship && <p className="academy-form-help">This saves a payment draft. Your registration is submitted only after Paystack confirms the correct payment and you complete the final step. Already paid? Open your Paystack return link in the same browser or <a href="/contact">contact admissions</a> with your reference before paying again.</p>}
      </form>
      <aside className="academy-registration-summary"><span className="academy-eyebrow">Your learning plan</span><h2>{course?.title || "Your next chapter"}</h2>
        ...
        <p>All fees shown are in Nigerian naira (NGN).</p><a href={scholarship ? `/register${course ? `?course=${course.id}` : ""}` : `/scholarship${course ? `?course=${course.id}` : ""}`}>{scholarship ? "Prefer full tuition? Register here →" : "Looking for a scholarship? Apply here →"}</a>
      </aside>
    </div>}<Footer />
  </main>;
}
