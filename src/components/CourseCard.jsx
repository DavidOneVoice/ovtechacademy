  ...
  return <article id={course.id} className="academy-course-card">
    ...
    <div className="academy-course-body">
      ...
      <dl className="academy-course-facts">
        ...
        <div><dt>Full tuition</dt><dd>{fees ? fees.tuition : <PricingStatus />}</dd></div>
        <div><dt>With scholarship</dt><dd>{fees ? <>{fees.scholarship} <small>({fees.scholarshipPercent} support)</small></> : "—"}</dd></div>
      </dl>
      ...
      <div className="academy-course-actions">
        <a target="_blank" rel="noopener noreferrer" className="academy-outline-button" href={`/courses/${course.id}`}>Explore this course ↗</a>
        ...
        <a target="_blank" rel="noopener noreferrer" className="academy-button academy-button-secondary" href={`/register?course=${course.id}`}>Register & Pay Full Tuition</a>
      </div>
    </div>
  </article>;
}
