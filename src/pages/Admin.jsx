...
const Admin = () => {
  ...
  const updateStatus = async (id, status, app) => {
    ...
    if (status === "Approved" && app.applicationType !== "tuition") {
      const currentFees = app.cohortId === COHORT.id && !app.paymentVerified
        ? getCoursePricing(app.courseId, app.detectedCountryCode) : null;
      await emailjs.send(
        ...
        import.meta.env.VITE_EMAILJS_TEMPLATE_ID,
        {
          ...
          mainMessage: `We are pleased to inform you that your OVTech Scholarship application has been approved. You have been selected to receive a ${currentFees?.scholarshipPercent || app.scholarshipPercent || pricing.NG.scholarshipPercent} scholarship for your chosen learning path.`,
          extraMessage: `To secure your slot, kindly complete your registration payment of ${currentFees?.scholarship || app.scholarshipFee || pricing.NG.scholarship} using the link below. Once payment is completed, our team will contact you with onboarding details.`,
          ctaText: "Registration Payment Link",
          ctaLink:
            app.cohortId === COHORT.id ? `${COHORT.website}/scholarship-payment?application=${app.id || id}` : app.scholarshipPaymentLink || `${COHORT.website}/contact`,
        },
